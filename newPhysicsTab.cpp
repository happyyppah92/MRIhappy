...
void NewPhysicsTab::on_copyImage()
{
	...
	QSize widsize = rawDataView->mygv->size();
	QImage imageAll(widsize.width() * 2, widsize.height(), QImage::Format_ARGB32);
	QPainter painter(&imageAll);

	rawDataView->mygv->render(&painter, QRect(0, 0, widsize.width(), widsize.height()));   //关键函数
	MRIdataView->mygv->render(&painter, QRect(widsize.width(), 0, widsize.width(), widsize.height()));   //关键函数
