...
#include "./myProcess/myFFTProcess.h"
#include <QDebug>
	...
	: QWidget(parent)
{
	...
	connect(newBtn, SIGNAL(clicked()), this, SLOT(on_newBtnClicked()));
	...
	connect(kspara->ui.pixchangebtn, SIGNAL(clicked()), this, SLOT(on_pixchangebtnClick()));
	connect(kspara->ui.resetbtn, SIGNAL(clicked()), this, SLOT(on_resetbtnClick()));

	connect(fftDatagraphviewer->mygv,SIGNAL(pixSelect_signal(QPointF)),this,SLOT(on_pixChange(QPointF)));
...
if (isFFTprocessed == false)
{
	if (curJobItem.curJobResult.size()>0)
	{
		...
		myAnnoTL.itemsStr.append(curJobItem.getJobSeqDiscrip());
...
void NewKspaceTab::on_pixchangebtnClick()
{
	fftDatagraphviewer->mygv-> enablePixchangeMode(true);
}

void NewKspaceTab::on_resetbtnClick()
{
	para.pixchange.clear();
	updateImages();
}

//点操作
void NewKspaceTab::on_pixChange(QPointF pos)
{
	int imageWidth = curJobItem.curJobData.widthImage;
	int imageHeight = curJobItem.curJobData.heightImage;
	QPoint p = pos.toPoint();
	if (p.x() > 0 && p.y() > 0 && p.x() < imageHeight && p.y() < imageWidth)
	{
		para.isPixchange = true;
		QPair<QPoint, double> temp;
		temp.first= p;//位置
		temp.second= kspara->ui.pixchangevalue->value();//值
		para.pixchange.append(temp);
		updateImages();
	}
}

void NewKspaceTab::on_newBtnClicked()
{
	curJobItem.curJobPara = curpara;
	curJobItem.curJobData = curData;
	curJobItem.curJobData.PDmap.clear();//清楚掉数据，节约内存
	curJobItem.curJobData.T1map.clear();
	curJobItem.curJobData.T2map.clear();
	curJobItem.curJobData.T2Startmap.clear();
	curJobItem.curJobResult = curImage.resultReal;
	myJobList.append(curJobItem);
	jobTable->addNewJob(myJobList);
}
...
void NewKspaceTab::on_copyImage()
{
	...
	QSize widsize = rawDatagraphviewer->mygv->size();
	QImage imageAll(widsize.width() * 2, widsize.height(), QImage::Format_ARGB32);
	QPainter painter(&imageAll);

	rawDatagraphviewer->mygv->render(&painter, QRect(0, 0, widsize.width(), widsize.height()));   //关键函数
	fftDatagraphviewer->mygv->render(&painter, QRect(widsize.width(), 0, widsize.width(), widsize.height()));   //关键函数
