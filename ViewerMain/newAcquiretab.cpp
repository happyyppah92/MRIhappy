...
NewAquireTab::NewAquireTab(QWidget *parent)
{
	...
	viewerLayout->addWidget(graphviewer1);
	viewerLayout->addWidget(graphviewer2);
	viewerLayout->addWidget(graphviewer3);
	viewerLayout->setStretchFactor(graphviewer1, 1);
	viewerLayout->setStretchFactor(graphviewer2, 1);
	viewerLayout->setStretchFactor(graphviewer3, 1);
...
int NewAquireTab::updateData(dataMode current)
{
	...
	if (current.dataType == dataTypeEnum::MAP2D || current.dataType == dataTypeEnum::MAP3D) {
		...
		tempjob.setJobSeqDiscrip("PD value");
		tempjob.setJobHead("");
		...
		tempjob.newJob("", QDateTime::currentDateTime(), curData, curpara);
		tempjob.setJobSeqDiscrip("T1 value");
		tempjob.setJobHead("");
		...
		tempjob.newJob("", QDateTime::currentDateTime(), curData, curpara);
		tempjob.setJobSeqDiscrip("T2 value");
		tempjob.setJobHead("");
...
void NewAquireTab::on_startButton_clicked()
{
	...
	curDiscription = curpara.Discription;
	tempjob.newJob(sqename, QDateTime::currentDateTime(), curData, curpara);//jobclass会自动写
	if (curpara.Seqtype == seqTypeEnum::SE)
	{
		if (curDiscription.size() == 0) {//
			tempjob.setJobSeqDiscrip(sqename + "->TE:" + QString::number(curpara.TE) + "ms,TR:" + QString::number(curpara.TR) + "ms");
		}
		else {
			curDiscription.replace("#TE", QString::number(curpara.TE));
			curDiscription.replace("#TR", QString::number(curpara.TR));
			tempjob.setJobSeqDiscrip(curDiscription);
		}
	}
	else if (curpara.Seqtype == seqTypeEnum::IR)
	{
		if (curDiscription.size() == 0)
		{//
			tempjob.setJobSeqDiscrip(sqename + "->TE:" +
		...
		{
			curDiscription.replace("#TE", QString::number(curpara.TE), Qt::CaseInsensitive);
			curDiscription.replace("#TR", QString::number(curpara.TR), Qt::CaseInsensitive);
			curDiscription.replace("#TI", QString::number(curpara.TI), Qt::CaseInsensitive);
			tempjob.setJobSeqDiscrip(curDiscription);
		}
	}
...
void NewAquireTab::showGraphviewwerByJob(myJobItem job)
{
	...
	myAnnoTL.itemsStr.append(job.getJobSeqDiscrip());
...
void NewAquireTab::on_linkAllcopyImage()
{
	...
	QSize widsize = graphviewer1->size();
	QImage imageAll(widsize.width() * 3, widsize.height(), QImage::Format_ARGB32);
	QPainter painter(&imageAll);

	graphviewer1->render(&painter, QRect(0, 0, widsize.width(), widsize.height()));   //关键函数
	graphviewer2->render(&painter, QRect(widsize.width(), 0, widsize.width(), widsize.height()));   //关键函数
	graphviewer3->render(&painter, QRect(widsize.width() * 2, 0, widsize.width(), widsize.height()));   //关键函数
